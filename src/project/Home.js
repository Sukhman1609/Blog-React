      ...
      <div className='home ' >
        ...
        <div className='theLatest'>
          ...
          {ContextData.filter((item)=> item.id%4===0 && item.id > 21 && item.id<25).map((item,index)=>{
            console.log(item);
            return(<>
            <div className='subTheLatestArticles'><div className='imgArticles'><NavLink to={`/detail/${item.id}`} style={{color:'black', textDecoration:'none'}} className='Navlink'><img src={item.bollywoodimg} alt={item.name} className='subLatest' id='subArticles'/></NavLink></div>
